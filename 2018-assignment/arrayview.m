...
% 1. write some code that checks the dimensions of the input argument
%
...
% .  ndims checks for the number of dimensions - length(size(data))
%       
% .  Use if and elseif, as well as == (Equal)
%        --> to check for appropriate dimensions of data array
%        
% .  Send error(msg) if input data is neither 3d or 4d
 

if ndims(data) == 3                     
     disp('h1: data set is already 3d')
                                                                                                              
elseif ndims(data) == 4              
    disp('h2: data set is 4d - take the mean')
    m = mean(data,4);
...
%% Return Slice
% . returnSlice1 is the fixed version of returnSlice that checks
%        that the input array is suitable

% . this will enable the function returnSlice1 to be used in contexts 
%    outside of the current script
...
s = returnSlice1(data, sliceNum, orientation); % now grab a slice using updated returnSlice1
